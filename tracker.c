...
void LoadConfigFile(struct TConfig *Config)
{
	...
	ReadBoolean(fp, "enable_hcsr04", -1, 0, &(Config->EnableHCSR04));
	if (Config->EnableHCSR04)
	{
		printf("HCSR04 Enabled\n");
	}
...
int main(void)
{
	...
	struct stat st = {0};
	struct TGPS GPS;
	pthread_t PredictionThread, LoRaThread, APRSThread, GPSThread, DS18B20Thread, ADCThread, CameraThread, BMP085Thread, BME280Thread, LEDThread, LogThread, PipeThread, VEML6070Thread, HCSR04Thread;
	...
	*Config.Channels[1].SSDVFolder = '\0';										// No folder for APRS images
	...
	if (Config.EnableHCSR04)
	{
		if (pthread_create(&HCSR04Thread, NULL, HCSR04Loop, &GPS))
		{
			fprintf(stderr, "Error creating HCSR04 thread\n");
			return 1;
		}
	}
