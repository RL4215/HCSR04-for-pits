...
int BuildSentence(unsigned char *TxLine, int Channel, struct TGPS *GPS)
{
	...
	int LoRaChannel;
	char TimeBuffer[12], ExtraFields1[20], ExtraFields2[20], ExtraFields3[20], ExtraFields4[64], ExtraFields5[32], ExtraFields6[32], ExtraFields8[32], ExtraFields9[32], *ExtraFields7;
	...
	if (Config.EnableVEML6070)
	{
		sprintf(ExtraFields9, ",%.1f", GPS->speed_sound);
	}
	...
	else
	{
			...
			sprintf((char *)TxLine, "$$%s,%d,%s,%7.5lf,%7.5lf,%5.5" PRId32  ",%d,%d,%d,%3.1f%s%s%s%s%s%s%s%s%s%s",
				Config.Channels[Channel].PayloadID,
				Config.Channels[Channel].SentenceCounter,
				TimeBuffer,
				GPS->Latitude,
